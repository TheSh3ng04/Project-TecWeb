...
let playerHasMoved = false;      // NEW: Track if player has made a move this turn
let canRoll = true;              // NEW: Track if player can roll
let mustPass = false;
...
function handlePlayerClick(e) {
    ...
    selectedPiece = null;
    playerHasMoved = true;  // Mark that player has moved
    refreshBoard();
    clearHighlights();
    
    const lastRoll = rolledValue;  // Store the roll value before resetting
    
    // Check if player gets an extra roll (1, 4, or 6)
    if ([1, 4, 6].includes(lastRoll)) {
        canRoll = true;  // Player can roll again
        rolledValue = 0;  // Reset roll value so player must roll again
        updateRollButtonState();
        document.getElementById("RollOutput").textContent = `Rolled ${lastRoll} Roll again!`;
        console.log("Extra roll earned!");
    } else {
        canRoll = false;  // Player must pass, cannot roll
        rolledValue = 0;
        updateRollButtonState();
        document.getElementById("RollOutput").textContent = "Pass your turn";
        mustPass = true;
    }
...
// NEW: Update roll button visual state
...
// --- ROLL BUTTON HANDLER (for player only) ---
function roll() {
    // Check if player can roll
    if (!canRoll) {
        alert("You cannot roll! You must pass your turn.");
        return;
    }
    
    // If already rolled and haven't moved, check if there are valid moves
    if (rolledValue !== 0 && !playerHasMoved) {
        const possibleMoves = playableBoard.get_possible_moves(currentPlayer, rolledValue);
        
        if (possibleMoves.length > 0) {
            // Valid moves exist - player MUST move first
            alert("You have valid moves! You must move before rolling again.");
            return;
        } else {
            // No valid moves - check if this roll grants an extra roll
            if ([1, 4, 6].includes(rolledValue)) {
                // Can roll again since no moves available but earned extra roll
                console.log(`No valid moves with ${rolledValue}, rolling again...`);
            } else {
                // No valid moves and no extra roll - must pass
                alert("No valid moves available. You must pass your turn.");
                canRoll = false;
                updateRollButtonState();
                updateTurnDisplay();
                return;
    ...
    rolledValue = rng();
    playerHasMoved = false;  // Reset move flag for new roll
    ...
    // After rolling, check if there are any valid moves
    const possibleMoves = playableBoard.get_possible_moves(currentPlayer, rolledValue);
    if (possibleMoves.length === 0) {
        console.log(`No valid moves for ${currentPlayer} with roll ${rolledValue}`);
        // If this roll grants extra roll, player can roll again
        if ([1, 4, 6].includes(rolledValue)) {
            const rollDiv = document.getElementById("RollOutput");
            if (rollDiv) {
                rollDiv.textContent = `${currentPlayer} rolled ${rolledValue} - No valid moves, roll again!`;
            }
        } else {
            // No extra roll, must pass
            const rollDiv = document.getElementById("RollOutput");
            if (rollDiv) {
                rollDiv.textContent = `${currentPlayer} rolled ${rolledValue} - No valid moves, must pass.`;
            }
            canRoll = false;
            updateRollButtonState();
            updateTurnDisplay();
...
// --- AI ROLL (no restrictions) ---
function aiRoll() {
    rolledValue = rng();
    console.log(`AI (${AISymbol}) rolled:`, rolledValue);
    updateSticks(rolledValue);
    
    const rollDiv = document.getElementById("RollOutput");
    if (rollDiv) {
        rollDiv.textContent = `${AISymbol} rolled ${rolledValue}`;
    }
    return rolledValue;
}
...
function updateTurnDisplay() {
    ...
    if (turnDiv) {
        ...
        // NEW: Show roll status
        if (!canRoll && playerHasMoved) {
            turnDiv.textContent += " (Must Pass)";
        }
    }
}
...
function passTurn() {
    ...
    // RULE 1 — Cannot pass without rolling first
    if (rolledValue === 0 && !mustPass) {
        alert("You must roll before passing your turn.");
        return;
    }

    const possibleMoves = playableBoard.get_possible_moves(currentPlayer, rolledValue);

    // RULE 2 — Cannot pass if valid moves exist
    if (possibleMoves.length > 0) {
        alert("You still have valid moves! You must make a move before passing.");
        return;
    }

    // RULE 3 — Cannot pass if roll grants an extra turn (1,4,6)
    if ([1, 4, 6].includes(rolledValue)) {
        alert(`You rolled a ${rolledValue}, which grants an extra turn. You must roll again.`);
        return;
    }
...
// Helper: end AI turn without invoking player-only pass checks
function endAITurn() {
    switchTurn();                // switch currentPlayer to human
    playerHasMoved = false;     // reset move flag for new player
    canRoll = true;
    updateRollButtonState();
    updateTurnDisplay();
    updateSticks(0);
    rolledValue = 0;
    console.log("AI turn ended, switched to", currentPlayer);
}
...
async function gameloop() {
    ...
    while (!playableBoard.is_game_won()) {
        ...
        if (currentPlayer === playerSymbol) {
            // waitForPass() will only resolve when the human legitimately passes
            // (it uses the player pass checks in passTurn()).
            await waitForPass();
            // After waitForPass returns, loop repeats and either human acted or passed,
            // so continue to top of while to check game state / turn.
            continue;
        }
        ...
        // ------------------------------
            ...
            // We'll allow ONE retry if AI rolls an "extra" number but has no moves.
            // Use a simple counter to prevent infinite retry chains.
            let retryAllowed = true;

            while (aiKeepPlaying && !playableBoard.is_game_won()) {

                const currentRoll = aiRoll();  // sets global rolledValue as well
                await new Promise(res => setTimeout(res, 1500));

                // IMPORTANT: ask for possible moves using the roll we just produced
                const possibleMoves = playableBoard.get_possible_moves(AISymbol, currentRoll);

                // --- CASE A: AI HAS NO MOVES ---
                if (possibleMoves.length === 0) {

                    // If roll grants an extra-turn (1,4,6) AND we haven't used the retry yet,
                    // allow exactly one retry roll.
                    if ([1, 4, 6].includes(currentRoll) && retryAllowed) {
                        console.log(`AI rolled ${currentRoll} but has no moves → using ONE retry roll`);
                        // consume the retry allowance and roll again
                        retryAllowed = false;
                        continue; // loop back to roll again
                    }

                    // Otherwise AI must pass (no moves, no retry left)
                    console.log("AI has no valid moves and cannot retry → ending AI turn");
                    aiKeepPlaying = false;
                    break;
                }

                // --- CASE B: AI HAS AT LEAST ONE MOVE ---
                // Make a move (your AI selects a move and returns new board)
                const root = new AI(playableBoard, AISymbol);
                const [evalScore, newBoard] = root.Run(AISymbol, currentRoll);
                playableBoard = newBoard;
                refreshBoard();

                await new Promise(res => setTimeout(res, 1000));

                if (playableBoard.is_game_won()) {
                    aiKeepPlaying = false;
                    break;
                }

                // If AI actually moved and the roll grants an extra turn -> continue
                if ([1, 4, 6].includes(currentRoll)) {
                    console.log(`AI rolled ${currentRoll} and made a move → extra turn granted`);
                    // On a successful move+extra-roll, we SHOULD allow further extra turns.
                    // Reset retryAllowance because a move was made (future no-move extra-rolls
                    // should get one retry again).
                    retryAllowed = true;
                    aiKeepPlaying = true;
                    // continue looping to roll again
                } else {
                    // No extra-turn number -> finish AI's turn
                    aiKeepPlaying = false;
                }
            } // end AI inner while

            // Instead of calling the human passTurn (which enforces player-only checks),
            // we explicitly end the AI turn here.
            if (!playableBoard.is_game_won()) {
                endAITurn();
            }
        } // end AI branch
...
drawBoard();
enablePlayerInput();
updateRollButtonState();  // NEW: Initialize roll button state
gameloop();